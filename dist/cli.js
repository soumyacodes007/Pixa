...
program
    .name("algopay")
    .description("Agentic payment wallet for Algorand — Stripe for AI Agents")
    .version("0.1.0")
    .option("--network <network>", "Algorand network (testnet or mainnet)", undefined)
    .option("--json", "Output in JSON format", false)
    .hook("preAction", (thisCommand) => {
    const opts = thisCommand.opts();
    const config = getConfig();
    // Resolve network: CLI flag > config > default (testnet)
    const network = opts.network ?? config.get("defaultNetwork") ?? "testnet";
...
program
    ...
    .action(async (amount, recipient, cmdOpts) => {
    const address = authClient.getWalletAddress();
    ...
    const asset = cmdOpts.asset.toUpperCase();
...
program
    ...
    .action(async (amount, from, to, cmdOpts) => {
    const address = authClient.getWalletAddress();
    const sessionToken = authClient.getSessionToken();
    if (!address || !sessionToken) {
        console.error("Not authenticated. Run: algopay auth login <email>");
    ...
    try {
        ...
        const quote = await vestige.getSwapQuote(fromAssetId, toAssetId, parsedAmount, fromDecimals, toDecimals, slippage, network);
        ...
        if (opts.json) {
            ...
            for (const tx of history) {
                ...
                try {
                    ...
                    if (nfd)
                        counterLabel = `${nfd} (${tx.counterparty.slice(0, 8)}...)`;
                }
                catch { /* offline */ }

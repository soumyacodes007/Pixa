 ...
 */
export function parsePaymentChallenge(headers, serviceUrl) {
    // Try X-Payment header (GoPlausible style)
    const xPayment = headers["x-payment"] ?? headers["X-Payment"];
    if (xPayment) {
        try {
            const parsed = JSON.parse(xPayment);
            const price = parsed.price_usdc ??
                parsed.price?.replace?.("$", "").replace?.(" USDC", "") ??
                parsed.amount ?? 0;
            const payTo = parsed.pay_to ?? parsed.payTo ?? parsed.wallet_address ?? "";
            const network = parsed.network === "algorand-mainnet"
                ? "mainnet"
                : "testnet";
            if (!payTo)
                return null;
...
export async function payAndFetch(opts) {
    const { serviceUrl, senderAddress, sessionToken, network, maxPrice = 10.0, // default: refuse payments > $10 USDC
    requestOptions = {}, } = opts;
    ...
    let group;
    try {
        group = await buildUsdcTransfer(senderAddress, challenge.payToAddress, challenge.price, ep.usdcAssetId, senderAddress, // backend address (same for mock)
        challenge.network);
    }
    catch (err) {
    ...
    let signResult;
    try {
        signResult = await intermezzo.signTransactions(group.unsignedTxns, group.userTxIndices, sessionToken);
    }
    catch (err) {
    ...
    const authPayload = Buffer.from(JSON.stringify({ txId, network: challenge.network, asset: "USDC" })).toString("base64");

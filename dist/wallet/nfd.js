 ...
 */
export function isNfdName(input) {
 ...
 */
export async function resolveNfdToAddress(name, network = "mainnet") {
    ...
    try {
        ...
        if (res.status === 404 || res.status === 400)
            return null;
        if (!res.ok)
            return null; // treat all other errors as 'not found' gracefully
        ...
        if (err.message.includes("NFD API error"))
            throw err;
 ...
 */
export async function resolveAddressToNfd(address, network = "mainnet") {
    const baseUrl = network === "mainnet" ? NFD_API_MAINNET : NFD_API_TESTNET;
    try {
        const res = await fetch(`${baseUrl}/nfd/lookup?address=${address}&view=tiny&allowUnverified=false`, { headers: { "Accept": "application/json" } });
        if (res.status === 404)
            return null;
        if (!res.ok)
            return null;
 ...
 */
export async function smartResolve(input, network) {
    if (!isNfdName(input)) {

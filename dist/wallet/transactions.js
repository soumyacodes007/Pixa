 ...
 */
export async function buildFeePooledGroup(userTxns, backendAddress, network) {
 ...
 */
export async function buildUsdcTransfer(senderAddress, recipientAddress, usdcAmount, usdcAssetId, backendAddress, network) {
    const algod = createAlgodClient(network);
    const params = await algod.getTransactionParams().do();
 ...
 */
export async function buildAlgoTransfer(senderAddress, recipientAddress, algoAmount, backendAddress, network) {
    const algod = createAlgodClient(network);
    const params = await algod.getTransactionParams().do();

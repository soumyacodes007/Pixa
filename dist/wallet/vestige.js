...
// --- Helpers ---
function vestigeHeaders() {
    return {
        "User-Agent": "algopay/1.0",
        "Accept": "application/json",
    };
}
async function vestigeFetch(path) {
    const url = `${VESTIGE_API}${path}`;
    const res = await fetch(url, { headers: vestigeHeaders() });
    if (!res.ok) {
        throw new Error(`Vestige API error: ${res.status} for ${path}`);
    }
...
export async function getAssetPrice(assetId) {
    ...
    const data = await vestigeFetch(`/asset/${assetId}/price?currency=usd`);
    return {
        assetId,
...
export async function getAlgoPrice() {
    // ALGO price is ASA 0
    try {
        const price = await vestigeFetch(`/asset/0/price?currency=usd`);
 ...
 */
export function resolveAssetId(nameOrId, network) {
    ...
    if (upper === "USDT")
        return WELL_KNOWN_ASSETS.USDT;
    if (upper === "WETH")
        return WELL_KNOWN_ASSETS.WETH;
    throw new Error(`Unknown asset: "${nameOrId}". Use ALGO, USDC, USDT, WETH, or an ASA ID number.`);
}
// --- Swap Quote ---
/**
 ...
 */
export async function getSwapQuote(fromAssetId, toAssetId, fromAmount, // in display units (e.g., 1.5 USDC)
fromDecimals, toDecimals, slippagePct = 1.0, network = "mainnet") {
    const microAmount = Math.round(fromAmount * Math.pow(10, fromDecimals));
    try {
        // Vestige swap/v4 quote endpoint
        const data = await vestigeFetch(`/swap/v4?asset_id_from=${fromAssetId}&asset_id_to=${toAssetId}&amount=${microAmount}&slippage=${slippagePct}`);
...
// --- Display Helpers ---
export function formatAssetName(assetId, network) {
    if (assetId === 0)
        return "ALGO";
    const ep = network === "mainnet" ? WELL_KNOWN_ASSETS : WELL_KNOWN_ASSETS;
    for (const [name, id] of Object.entries(ep)) {
        if (id === assetId)
            return name.replace("_TESTNET", "").replace("_MAINNET", "");
    }
    return `ASA-${assetId}`;
}
//# sourceMappingURL=vestige.js.map
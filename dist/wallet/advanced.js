 ...
 */
export async function getTransactionHistory(address, network, limit = 10) {
 ...
 */
export async function getAssetHoldings(address, network) {
 ...
 */
export async function buildOptInTransaction(address, assetId, network) {
    const algod = createAlgodClient(network);
...
export async function getNetworkStatus(network) {
    ...
    try {
        ...
        const lastRound = Number(s["last-round"] ?? s.lastRound ?? s["lastRound"] ?? 0);
        const catchupTime = Number(s["catchup-time"] ?? s.catchupTime ?? 0);
        const genesisId = String(s["genesis-id"] ?? s.genesisId ?? "unknown");
        return {
            network,
            healthy: lastRound > 0,
            lastRound,

 ...
 */
export function recordSpend(amount, asset) {
    spendHistory.push({
        timestamp: Date.now(),
        amount,
        asset: String(asset),
 ...
 */
export function checkSpendingLimit(amount, asset) {
...
export function allowAddress(address) {
    allowlist.add(address.toUpperCase());
}
function checkAllowBlocklist(recipientAddress) {
...
function checkSelfSend(senderAddress, recipientAddress) {
...
function checkZeroAmount(amount) {
    if (amount <= 0) {
 ...
 */
export function runGuardrails(ctx) {
    const checks = [

...
export interface WebhookPayload {
  ...
  data: {
    ...
    balance?: {
      algo: number;
      assets: Array<{ id: number; amount: number }>;
    };
    limit?: {
      asset: string;
      amount: number;
...
export function addWebhook(
  url: string, 
  events: WebhookEvent[] = ["transaction.sent", "transaction.received"],
  secret?: string
): Webhook {
  // Validate URL
  try {
...
export function notifyTransactionReceived(
  txId: string,
  amount: number,
  asset: string,
  sender: string,
...
export function notifyBalanceUpdated(
  balance: WebhookPayload["data"]["balance"],
  network: string
): void {
...
export function notifyLimitExceeded(
  asset: string,
  amount: number,
  used: number
): void {
